...
public class EventDAO {
    ...
    public List<Event> getEvents(int userId){
        ...
        try(Connection con = connectionManager.getConnection()){
            try(PreparedStatement ps = con.prepareStatement("SELECT *, (SELECT COUNT(*) FROM EventCoordinators WHERE EventID = Events.EventID) AS CoordinatorCount FROM Events INNER JOIN EventCoordinators ON Events.EventID = EventCoordinators.EventID WHERE EventCoordinators.UserID = ?")){
                ps.setInt(1, userId);
